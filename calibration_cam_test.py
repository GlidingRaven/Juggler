...
Cord_finder = Predictors.Cord_finder(model_name)
# models = []
...
myGUI = Plotter.CustomMainWindow(queue_size = 200, y_limit = 100, start_offset = 200, ylabel='z vel')
    ...
    if ret1 and ret2:
        ...
        if circles1 is not None and circles2 is not None:
            ...
            if vector[0] < 15 and vector[1] < 15: # filter out Outliers
                vector_avg.add('x_vel', vector[0])
                vector_avg.add('y_vel', vector[1])
            ...
            Trigger.detect_velocity_decay(vector[2], haha)
            ...
            mySrc.data_signal.emit(cord_int[2])
